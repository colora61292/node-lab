...
Controller.prototype.saveModelAttr = function(model) {
    var key = require('kw').util.encrypt(this.name,this.secret);
    this.req.session[key] = JSON.stringify(model);
};

Controller.prototype.loadModelAttr = function(isFlash) {
    var key = require('kw').util.encrypt(this.name,this.secret);
    if(!this.req.session[key]){
        return null;
    }
    var model = JSON.parse(this.req.session[key]);
    if(isFlash==true){
